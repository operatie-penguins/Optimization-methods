...
//    static boolean writeEnabled;
	static Font[] fonts = {
		...
		new Font("SansSerif", 3, 8) // мелкие индексы
	};
	...
	public static void setFontSizes(int a, int b, int c) {
		...
		fonts[2] = new Font("SansSerif", 3, c);
...
*/
				...
				text1 = s.substring(i + 1, iEnd);
				dy = -7;
				x = drawSubstring(text1, x-4, y + dy, sy + 1);
			}
			else if(sym == '/') {
				text1 = s.substring(i + 1, iEnd);
				i = iEnd + 2; //{f}{g}
				...
				text2 = s.substring(i, iEnd);
				dy = 5;
				int xx1 = drawSubstring(text1, x, y - (int) r.getHeight() / 2+1, sy);
				int xTemp = x;
				x = drawSubstring(text2, x, y + (int) r.getHeight() / 2+1, sy);
				if (xx1 > x) {
					x = xx1;
				}
				g2d.drawLine(x, y - (int) r.getHeight() / 2+3, xTemp-1, y - (int) r.getHeight() / 2+3);
                                g2d.drawLine(x, y - (int) r.getHeight() / 2+4, xTemp-1, y - (int) r.getHeight() / 2+4);
			}
