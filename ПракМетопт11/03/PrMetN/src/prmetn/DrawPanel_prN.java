...
public class DrawPanel_prN extends JPanel {
  private Color ColorFon=new Color(247,247,255);//Цвет фона
//панели
  private Color ColorPanelFon=new Color(255,255,255);//Цвет фона панелей
  private Color ColorPanelPamka=new Color(157,187,255);//Цвет обрамления
//Надписи(напр. "Графическая иллюстрация" и т.д.)
  private Color ColorSubTitle=new Color(180,0,60);//Цвет
  private Font FontSubTitle=new java.awt.Font("Dialog",3,12);//Шрифт
//Поле "Решение"
...
////////////////////////
    ...
    ///////////////   1  ////////////////////////////////////////////
                        case 1:
                          c = "  Задаём x^{0} = (" + f.x1Str() + "; " + f.x2Str() +
                              "), \u03b5_{1} = " + f.eps1Str()+">0"  +
                              ", \u03BC^{0} = "+f.Vektor[0]+", M - предельное  число " ;
                           b = " итераций. Строим линии уровня функции f(x).    Замечание:n{}"+
                               "\u03BC^{0} выбирается как минимум на порядок больше, чем самый большой n{}"+
                              "элемент матрицы Н(x^{0}), а в ряде стандартных программ полагается \u03BC^{0} = 10^{4}." ;
                              nextStep = step + 1;
                          break;
                        case 2:
                          ...
                          break;
                        case 3:
                          c = "  Найдем матрицу Гессе";
                          b = "H(x) = #{f_{x_{1}x_{1}}}{f_{x_{1}x_{2}}}{f_{x_{2}x_{1}}}{f_{x_{2}x_{2}}} = " + f.gesseStr();
                          nextStep = step + 1;
                          break;
                       case 4:
                         c = "  Положим k = "+k+", \u03BC^{k} = \u03BC^{"+k+"} = "+f.Vektor[0]+"";
                         b = "";
                         nextStep = step + 1;
                         break;

                        case 5:
                          c = "  Вычислим градиент в точке x^{" + k + "}:";
                          b = "\u2207f(x^{" + k + "}) = #{" + f.g1Str() + "}{" + f.g2Str() + "}";
                          nextStep = step + 1;
                          break;

                        case 6:
                         // b = "Проверим выполнение критерия окончания \u2225\u2207f(x^{" + k + "})\u2225 \u2264 \u03b5_{1}:";
                          if (f.gNorm() > f.eps1)
                          {
                             //c = "Проверим выполнение критерия окончания \u2225\u2207f(x^{" + k + "})\u2225 \u2264 \u03b5_{1}:";
                            c ="  Проверим выполнение критерия окончания \u2225\u2207f(x^{" + k + "})\u2225 \u2264 \u03b5_{1}:n{}"+
                                "\u2225\u2207f(x^{" + k + "})\u2225 = "+f.gNormStr() + " > " +
                            f.eps1Str() + "; условие не выполняется, переходим n{}к следующему шагу.";
                             nextStep = step + 1;
                              b="";
                           }
                          else
                           {   if (f.gNorm() <= f.eps1)
                               ...
                               c= "  Проверим выполнение критерия окончания \u2225\u2207f(x^{" + k + "})\u2225 \u2264 \u03b5_{1}:n{}"+
                               "\u2225\u2207f(x^{" + k + "})\u2225 = "+f.gNormStr() + " \u2264 " +
                              f.eps1Str() + "; условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +
                               "} = #{" + f.x1Str() + "}{" + f.x2Str() +
                               "}e{} - искомое приближение к решению.";
                               nextStep =-1;
                               b="";
                               }
                         ...
                         case 7:
                        if ( k < M )
                        { c = " Проверим выполнение условия k \u2265 M: k= " + k + " < "+"M "+"n{}" +
                           " Условие не выполняется, значит переходим к следующему шагу.";
                        b = "";
                          nextStep = step + 1;
                        }
                        else

                        {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
                            " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
                              "}e{} - искомое приближение к решению.";
                              nextStep =-1;
                              b="";
                          }
                       break;
                        ...
                        case 8:
                          c = "  Вычисляем матрицу H(x^{" + k + "}) = " + f.hStr();
                          b="";
                          nextStep = step + 1;
                          break;

                        case 9:
                         c = "  Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(0);
                         b="";
                         nextStep = step + 1;
                         break;

                        case 10:
                          c = "  Вычисляем значение обратной матрицы, полученной на";
                         // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                         b = "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(0);
                          nextStep = step + 1;
                          break;
                        ...
                        case 13:
                       c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): ";
                       if (f.dFuncNorm_a() < f.dFuncNorm_b()) {
                          b = "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                              " Условия выполняется, поэтому полагаем k = k+1 = "+k+"+1 = "+(k+1)+" ,\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[0]/2 +
                             " n{}и переходим к шагу 5.";
                           nextStep = step + 1;
                        //k++;
                         ...
                         b = "  f(x^{" + (k + 1) + "}) = " +f.dFuncNormStr_a() + " > "+ f.dFuncNormStr_b() + " = f(x^{" + k + "})"+".n{}" +
                         " Условия не выполняется, поэтому полагаем \u03BC^{"+k+"} = 2 * \u03BC^{"+k+"}" +
                             " n{}и переходим к шагу 9.";
                         //f.setx1x2(f.x1New(), f.x2New());
                        nextStep = -1;
                        // k++;
                       }
    ...
    ///////    2               /////////////////// новый раздел//////////////////////////////////////
                   ...
                   k++;
                      f.setx1x2(f.x1New(k), f.x2New(k));
                       ...
                       c = "  Вычислим градиент в точке x^{" + k + "}:n{}"+
...
{   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
    " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
      ...
      b="";
                        ...
                        break;
                      case 18:
                      c = "  Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(1);
                      b="";
                      nextStep = step + 1;
                      break;
                      ...
                      // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                       b = "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(1);
                        nextStep = step + 1;
                        break;
                      ...
                      b = " d_{" + k + "} = -[H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} \u2207f(x^{" + k + "}) = - " + f.h_1_plusStr(1) + "#{" +
                       ...
                       break;
                        ...
                        c = "  Находим точку x^{" + (k + 1) + "} = x^{" + k + "} + d^{" + k + "} = " +
                         ...
                         case 22:
                         // b = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): ";
                           if (f.dFuncNorm_a() < f.dFuncNorm_b()) {
                            c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                            "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                            " Условия выполняется, поэтому полагаем k = k+1 = "+k+"+1 = "+(k+1)+" ,\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[1]/2 +
                            " n{}и переходим к шагу 5.";
                             nextStep = step + 1;
                            //  k++;
                              }
     ...
     //////////             3              //////////////// новый раздел//////////////////////////////////////
                  ...
                  {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
                      " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
                       ...
                       break;
                     case 27:
                     c = "  Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(2);
                     b="";
                     nextStep = step + 1;
                     break;
                     ...
                     // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                      b = "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(2);
                       nextStep = step + 1;
                       break;
                       ...
                       c = "  Находим точку x^{" + (k + 1) + "} = x^{" + k + "} + d^{" + k + "} = " +
                        ...
                        case 31:
                         c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): ";
                          if (f.dFuncNorm_a() < f.dFuncNorm_b()) {
                           b = "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                           " Условия выполняется, поэтому полагаем k = k+1 = "+ k +"+1 = "+(k+1)+" ,\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[2]/2 +
                           " n{}и переходим к шагу 5.";
                            nextStep = step + 1;
                            // k++;
                             }
                ...
                {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
                    " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
                      ...
                      b="";
                          ...
                          break;
                        case 36:
                        c = "  Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(3);
                        b="";
                        nextStep = step + 1;
                        break;

                        case 37:
                        c = "  Вычисляем значение обратной матрицы, полученной на n{}"+

                        // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                         "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(3);
                     b="";
                          nextStep = step + 1;
                          break;

                        case 38:
                        c = "  Теперь вычисляем:n{}"+
                          ...
                          c = "  Находим точку x^{" + (k + 1) + "} = x^{" + k + "} + d^{" + k + "} = " +
                           ...
                           // c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k+ "}): ";
                             if (f.dFuncNorm_a() < f.dFuncNorm_b()) {
                              c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k+ "}):n{} "+
                                  "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                              " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" ,\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[3]/2 +
                              " n{}и переходим к шагу 5.";
                          b="";
                               nextStep = step + 1;
                               // k++;
       ...
       {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
           " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
             ...
             b="";
                                ...
                                break;
                              case 45:
                              c = "  Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k+"}*E = " +f.h_plusStr(4);
                              b="";
                              nextStep = step + 1;
                              break;
                              ...
                              case 46:
                              c = "  Вычисляем значение обратной матрицы, полученной на n{}"+
                             "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(4);
                           b="";
                                nextStep = step + 1;
                                break;
                                ...
                                c = "  Находим точку x^{" + (k + 1) + "} = x^{" + k + "} + d^{" + k + "} = " +
                                 ...
                                 // c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): ";
                                   if (f.dFuncNorm_a() < f.dFuncNorm_b()) {
                                     c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}):n{} "+
                                      "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k+ "}) "+".n{}"+
                                    " Условия выполняется, поэтому полагаем k = k+1 = "+ k +"+1 = "+(k+1)+" ,\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[4]/2 +
                                    " n{}и переходим к шагу 5.";
                                b="";
                                     nextStep = step + 1;
                                     // k++;
 ...
 {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
     " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
       ...
       b="";
                                      ...
                                      break;
                                    case 54:
                                    c = "  Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(5);
                                    b="";
                                    nextStep = step + 1;
                                    break;
                                    ...
                                    "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(5);
                                b="";
                                      nextStep = step + 1;
                                      break;
                                     ...
                                     break;
                                      ...
                                      //  c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): ";
                                         if (f.dFuncNorm_a() < f.dFuncNorm_b()) {
                                           c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                            "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                          " Условия выполняется, поэтому полагаем k = k+1 = "+ k +"+1 = "+(k+1)+" ,n{}\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+
                                          f.Vektor[5]/2 + " и переходим к шагу 5.";
                                      b="";
                                           nextStep = step + 1;
                                           // k++;
                                            }
           ...
           {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
               " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
                 ...
                 b="";
                                                        ...
                                                        break;
                                                      case 63:
                                                      c = "  Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(6);
                                                      b="";
                                                      nextStep = step + 1;
                                                      break;
                                                    ...
                                                    // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                       "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(6);
                                                   b="";
                                                        nextStep = step + 1;
                                                        break;
                                                        ...
                                                        c = "  Находим точку x^{" + (k + 1) + "} = x^{" + k + "} + d^{" +
                                                         ...
                                                         // c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): ";
                                                           if (f.dFuncNorm_a() < f.dFuncNorm_b()) {
                                                             c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                            " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{}\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[6]/2 +
                         ...
                         b="";
                                               ...
                                               b="";
                                                nextStep = step + 1;
                                                 ...
                                                 break;
                                                  case 72:
                                                 c = "  Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(7);
                                                 b="";
                                                 nextStep = step + 1;
                                                 break;
                                                 ...
                                                 case 73:
                                                 c = "  Вычисляем значение обратной матрицы, полученной на n{}"+
                                               "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(7);
                                             b="";
                                                    nextStep = step + 1;
                                                      break;
                                        ...
                                        if (f.dFuncNorm_a() < f.dFuncNorm_b())
                                              ...
                                              c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                  "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                            " Условия выполняется, поэтому полагаем k = k+1 = "+ k +"+1 = "+(k+1)+" ,n{}\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[7]/2 +
                                          " и переходим к шагу 5.";
                                           b="";
                                         nextStep = step + 1;
                                          //k++;
                       ...
                       {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
                           " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
                             ...
                             nextStep =-1;
                                    ...
                                    break;
                                  case 81:
                                  c = "  Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(8);
                                  b="";
                                  nextStep = step + 1;
                                  break;
                                  ...
                                  // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                 "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(8);
                             b="";
                                  nextStep = step + 1;
                                  break;
                                  ...
                                  // c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): ";
                                   ...
                                   c = "   Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                       "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                    " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{}\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[8]/2 +
                                    " и переходим к шагу 5.";
                                b="";
                                    nextStep = step + 1;
                                  //k++;
                        ...
                        {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
                            " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
                              ...
                              nextStep =-1;
                                                   ...
                                                   break;
                                                 case 90:
                                                 c = "   Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(9);
                                                 b="";
                                                 nextStep = step + 1;
                                                 break;
                                                 ...
                                                 // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(9);
                                            b="";
                                                 nextStep = step + 1;
                                                 break;
                                                  ...
                                                  //c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): ";
                                                   ...
                                                   c = "   Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                      "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                   " Условия выполняется, поэтому полагаем k = k+1 = "+k+"+1 = "+(k+1)+" ,n{}\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[9]/2 +
                                                   " и переходим к шагу 5.";
                                               b="";
                                                   nextStep = step + 1;
                                                //k++;
                           ...
                           {   c= "       Проверим выполнение условия k \u2265 M: k = " + k + " < "+ M +" = M  " +"n{}"+
                               " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
                                 ...
                                 nextStep =-1;
                                            ...
                                            break;
                                        case 99:
                                          c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(10);
                                          b="";
                                          nextStep = step + 1;
                                          break;
                                    ...
                                    "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(10);
                                     b="";
                                          nextStep = step + 1;
                                          ...
                                          break;
                                           ...
                                           //c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): ";
                                            ...
                                            c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                               "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                            " Условия выполняется, поэтому полагаем k = k+1 = "+k+"+1 = "+(k+1)+" ,n{}\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[10]/2 +
                                            " и переходим к шагу 5.";
                                        b="";
                                            nextStep = step + 1;
                                         //k++;
                        ...
                        case 104:
                              ...
                              b="";
                                ...
                                " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
                                  ...
                                  break;
                                case 108:
                                c = "      Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(11);
                                b="";
                                nextStep = step + 1;
                                break;
                                ...
                                // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                 "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(11);
                               b="";
                                nextStep = step + 1;
                                break;
                                ...
                                c = "      Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                  " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" ,n{}\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[11]/2 +
                                  " и переходим к шагу 5.";
                              b="";
                                  nextStep = step + 1;
                               // k++;
                            ...
                            case 113:
                              ...
                              " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
                                ...
                                nextStep =-1;
                                  ...
                                  break;
                                case 117:
                                c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(12);
                                b="";
                                nextStep = step + 1;
                                break;
                                ...
                                // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                 "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(12);
                             b="";
                                nextStep = step + 1;
                                break;
                                ...
                                // c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): ";
                                 ...
                                 c = "      Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                  "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                  " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{}\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[12]/2 +
                                  " и переходим к шагу 5.";
                              b="";
                                  nextStep = step + 1;
                                //k++;
           ...
           " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
             ...
             nextStep =-1;
                                               ...
                                               case 125:
                                                                            ...
                                                                            break;
                                                                          case 126:
                                                                          c = "     Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(13);
                                                                          b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                          "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(13);
                                                                      b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                           ...
                                                                           c = "      Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                           "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                            " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" ,n{}\u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[13]/2 +
                                                                            " и переходим к шагу 5.";
                                                                        b="";
                                                                            nextStep = step + 1;
                                                                         // k++;
      ...
      {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
          " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
            ...
            nextStep =-1;
                                  ...
                                  break;
                                case 135:
                                c = "      Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(14);
                                b="";
                                nextStep = step + 1;
                                break;
                                ...
                                // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                               "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(14);
                           b="";
                                nextStep = step + 1;
                                break;
                                ...
                                // c = "  Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): ";
                                 ...
                                 c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                     "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                  " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[14]/2 +
                                  " и переходим к шагу 5.";
                              b="";
                                  nextStep = step + 1;
                               // k++;
       ...
       {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
           " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
             ...
             nextStep =-1;
                                                                            ...
                                                                            break;
                                                                          case 144:
                                                                          c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(15);
                                                                          b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                          "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(15);
                                                                      b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                               "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                            " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[15]/2 +
                                                                            " и переходим к шагу 5.";
                                                                        b="";
                                                                            nextStep = step + 1;
                                                                         // k++;
      ...
      {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
          " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
            ...
            nextStep =-1;
                                                                            ...
                                                                            break;
                                                                          case 153:
                                                                          c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(16);
                                                                          b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                          "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(16);
                                                                      b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                               "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                            " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[16]/2 +
                                                                            " и переходим к шагу 5.";
                                                                        b="";
                                                                            nextStep = step + 1;
                                                                         // k++;
      ...
      {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
          " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
            ...
            nextStep =-1;
                                                                            ...
                                                                            break;
                                                                          case 162:
                                                                          c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(17);
                                                                          b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                          "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(17);
                                                                      b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                               "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                            " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[17]/2 +
                                                                            " и переходим к шагу 5.";
                                                                        b="";
                                                                            nextStep = step + 1;
                                                                         // k++;
        ...
        {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
            " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
              ...
              nextStep =-1;
                                                                            ...
                                                                            break;
                                                                          case 171:
                                                                          c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(18);
                                                                          b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                          "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(18);
                                                                      b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                               "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                            " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[18]/2 +
                                                                            " и переходим к шагу 5.";
                                                                        b="";
                                                                            nextStep = step + 1;
                                                                         // k++;
     ...
     {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
         " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
           ...
           nextStep =-1;
                                                                            ...
                                                                            break;
                                                                          case 180:
                                                                          c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(19);
                                                                          b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                          "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(19);
                                                                      b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                               "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                            " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[19]/2 +
                                                                            " и переходим к шагу 5.";
                                                                        b="";
                                                                            nextStep = step + 1;
                                                                         // k++;
      ...
      {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
          " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
            ...
            nextStep =-1;
                                                                            ...
                                                                            break;
                                                                          case 189:
                                                                          c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(20);
                                                                          b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                          "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(20);
                                                                      b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                               "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                            " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[20]/2 +
                                                                            " и переходим к шагу 5.";
                                                                        b="";
                                                                            nextStep = step + 1;
                                                                        //  k++;
      ...
      {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
          " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
            ...
            nextStep =-1;
                                                                            ...
                                                                            break;
                                                                          case 198:
                                                                          c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(21);
                                                                          b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                          "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(21);
                                                                      b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                               "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                            " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[21]/2 +
                                                                            " и переходим к шагу 5.";
                                                                        b="";
                                                                            nextStep = step + 1;
                                                                        //  k++;
       ...
       {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
           " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
             ...
             nextStep =-1;
                                                                            ...
                                                                            break;
                                                                          case 207:
                                                                          c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(22);
                                                                          b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                          "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(22);
                                                                      b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                               "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                            " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[22]/2 +
                                                                            " и переходим к шагу 5.";
                                                                        b="";
                                                                            nextStep = step + 1;
                                                                         // k++;
      ...
      {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
          " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
            ...
            nextStep =-1;
                                                                            ...
                                                                            break;
                                                                          case 216:
                                                                          c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(23);
                                                                          b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                          "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(23);
                                                                      b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                               "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                            " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[23]/2 +
                                                                            " и переходим к шагу 5.";
                                                                        b="";
                                                                            nextStep = step + 1;
                                                                        //  k++;
...
////////         25           ////////////////// новый раздел//////////////////////////////////////
      ...
      {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
          " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
            ...
            nextStep =-1;
                                                                            ...
                                                                            break;
                                                                          case 225:
                                                                          c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(24);
                                                                          b="";
                                                                          nextStep = step + 1;
                                                                          break;
                                                                          ...
                                                                          // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                          "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(24);
                                                                      b="";
                                                                          nextStep = step + 1;
                                                                          break;
  ...
  case 229:
                                                                          ...
                                                                          c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                               "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                            " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[24]/2 +
                                                                            " и переходим к шагу 5.";
                                                                        b="";
                                                                            nextStep = step + 1;
                                                                         // k++;
       ...
       {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
           " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
             ...
             nextStep =-1;
                                                                                                      ...
                                                                                                      break;
                                                                                                    case 234:
                                                                                                    c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(25);
                                                                                                    b="";
                                                                                                    nextStep = step + 1;
                                                                                                    break;
                                                                                                    ...
                                                                                                    // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                                                    "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(25);
                                                                                                b="";
                                                                                                    nextStep = step + 1;
                                                                                                    break;
                      ...
                      case 238:
                                                                                                    ...
                                                                                                    c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                                                         "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                                                      " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[25]/2 +
                                                                                                      " и переходим к шагу 5.";
                                                                                                  b="";
                                                                                                      nextStep = step + 1;
                                                                                                   // k++;
      ...
      {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
          " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
            ...
            nextStep =-1;
                                                                                                 ...
                                                                                                 break;
                                                                                               case 243:
                                                                                               c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(26);
                                                                                               b="";
                                                                                               nextStep = step + 1;
                                                                                               break;
                                                                                               ...
                                                                                               // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                                               "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(26);
                                                                                           b="";
                                                                                               nextStep = step + 1;
                                                                                               break;
                                                                                               ...
                                                                                               c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                                                    "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                                                 " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[26]/2 +
                                                                                                 " и переходим к шагу 5.";
                                                                                             b="";
                                                                                                 nextStep = step + 1;
                                                                                              // k++;
      ...
      {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
          " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
            ...
            nextStep =-1;
                                                                                                                                                                              ...
                                                                                                                                                                              break;
                                                                                                                                                                            case 252:
                                                                                                                                                                            c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(27);
                                                                                                                                                                            b="";
                                                                                                                                                                            nextStep = step + 1;
                                                                                                                                                                            break;
                                                                                                                                                                            ...
                                                                                                                                                                            // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                                                                                                                            "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(27);
                                                                                                                                                                        b="";
                                                                                                                                                                            nextStep = step + 1;
                                                                                                                                                                            break;
                                                                                                                                                                            ...
                                                                                                                                                                            c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                                                                                                                                 "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                                                                                                                              " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[27]/2 +
                                                                                                                                                                              " и переходим к шагу 5.";
                                                                                                                                                                          b="";
                                                                                                                                                                              nextStep = step + 1;
                                                                                                                                                                           // k++;
      ...
      {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
          " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
            ...
            nextStep =-1;
                                                                                                                                                                                                                                                           ...
                                                                                                                                                                                                                                                           break;
                                                                                                                                                                                                                                                         case 261:
                                                                                                                                                                                                                                                         c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(28);
                                                                                                                                                                                                                                                         b="";
                                                                                                                                                                                                                                                         nextStep = step + 1;
                                                                                                                                                                                                                                                         break;
                                                                                                                                                                                                                                                         ...
                                                                                                                                                                                                                                                         // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                                                                                                                                                                                                         "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(28);
                                                                                                                                                                                                                                                     b="";
                                                                                                                                                                                                                                                         nextStep = step + 1;
                                                                                                                                                                                                                                                         break;
                                                                                                                                                                                                                                                         ...
                                                                                                                                                                                                                                                         c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                                                                                                                                                                                                              "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                                                                                                                                                                                                           " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[28]/2 +
                                                                                                                                                                                                                                                           " и переходим к шагу 5.";
                                                                                                                                                                                                                                                       b="";
                                                                                                                                                                                                                                                           nextStep = step + 1;
                                                                                                                                                                                                                                                       //  k++;
     ...
     {   c= " Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
         " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
           ...
           nextStep =-1;
                                                                                                                                                                                                                                                                           ...
                                                                                                                                                                                                                                                                           break;
                                                                                                                                                                                                                                                                         case 270:
                                                                                                                                                                                                                                                                         c = "       Вычисляем матрицу H(x^{" + k + "})+\u03BC^{"+ k +"}*E = " +f.h_plusStr(29);
                                                                                                                                                                                                                                                                         b="";
                                                                                                                                                                                                                                                                         nextStep = step + 1;
                                                                                                                                                                                                                                                                         break;
                                                                                                                                                                                                                                                                         ...
                                                                                                                                                                                                                                                                         // b = "H^{-1}(x^{" + k + "}) = " + f.h_1Str()+"n{}"+
                                                                                                                                                                                                                                                                         "  предыдущем шаге:       [H(x^{" + k + "})+\u03BC^{"+ k +"}*E]^{-1} = " + f.h_1_plusStr(29);
                                                                                                                                                                                                                                                                     b="";
                                                                                                                                                                                                                                                                         nextStep = step + 1;
                                                                                                                                                                                                                                                                         break;
                                                                                                                                                                                                                                                                         ...
                                                                                                                                                                                                                                                                         c = "        Проверяем выполнение условия f(x^{" + (k + 1) + "}) < f(x^{" + k + "}): n{}"+
                                                                                                                                                                                                                                                                              "  f(x^{" + (k + 1) + "}) = " + f.dFuncNormStr_a() + " < "+ f.dFuncNormStr_b() + " = f(x^{" + k + "}) "+".n{}"+
                                                                                                                                                                                                                                                                           " Условия выполняется, поэтому полагаем k = k+1 = "+(k)+"+1 = "+(k+1)+" , n{} \u03BC^{"+(k+1)+"} = \u03BC^{"+k+"}/2 = "+f.Vektor[29]/2 +
                                                                                                                                                                                                                                                                           " и переходим к шагу 5.";
                                                                                                                                                                                                                                                                       b="";
                                                                                                                                                                                                                                                                           nextStep = step + 1;
                                                                                                                                                                                                                                                                       //  k++;
       ...
       {   c= "         Проверим выполнение условия k \u2265 M: k = " + k + " \u2265 "+ M +" = M  " +"n{}"+
           " Условие выполняется,n{}значит r{}x^{*} = x_{" + (k) +"} = #{" + f.x1Str() + "}{" + f.x2Str() +
             ...
             nextStep =-1;
                            ...
                            default: c = "неизвестнЫй науке текст!!";
                }
        }
...
//начальные данные
              ...
              sc(g, new int[]{1}, xOff+xLen+18+9, yOff+36);
                ...
                Stringer_prN.drawString("c{- начальная точка}", xOff+xLen+18+7+73, yOff + 15 );
                Stringer_prN.drawString("  c{-малое число}", xOff+xLen+80, yOff + 30 );
                Stringer_prN.drawString("\u03BC^{0} = "+ m, xOff+xLen+18+8, yOff+49);
                Stringer_prN.drawString("  c{-положительное число}", xOff+xLen+85, yOff + 47 );
                Stringer_prN.drawString(" M = "+M, xOff+xLen+18+5, yOff+65);
                Stringer_prN.drawString("  c{-количество итераций}", xOff+xLen+68, yOff + 65 );
...
//формулы
               ...
               Stringer_prN.drawString("k \u2265 M         (E - единичная матрица)", xOff+xLen+28+5, yOff+69+20+140);
...
// надписи
                ...
                g2d.drawString("Формулы: ", xOff+xLen+18+7, yOff+69+20-3);
                g2d.drawString("Графическая иллюстрация: ", xOff+7, yOff-3);
                g2d.drawString("Решение: ", xOff+7, yOff+yLen+20-3);
                g2d.drawString("Bеличины: ", xOff+xLen+18+7, yOff-3);
                g2d.drawString("Задача: ",  xOff+7, -34);
                g2d.drawString("Масштаб:", xOff+xLen+18+7, yOff+69+20+135+20-3-6+15);
//задание
                g2d.setFont(FontSolutionText);
                g2d.setColor(ColorSolutionText);
                g2d.drawString("Найти минимум функции ", 63, -34);
              //  g2d.drawString("методом Марквардта", 200-10, -20+5);
      ...
      if(fll==2)
        {
          ...
          g.drawString("Номер итерации",8+10,338-20);
        }
      if(fll==3)
        {
          g.setColor(ColorFon);
          ...
          g.setFont(FontGraphicXY);
          g.drawString("Номер шага",8+10+13,338-20);
        }
