...
public class BUKgarf {
        static Graphics2D g2d;
        static Font[] fonts = {
                new Font("SansSerif", 1, 10),	// основной текст
                new Font("SansSerif", 1, 7),   // глваный индекс
                new Font("SansSerif", 1, 7)     // мелкие индексы
        };
...
String U[], String D[], String strZ[],int fs, int count, int xx, int yy)
{
 ...
 int size=0, size1=0, size2=0, dl;
  ...
  BUKgarf.fonts[0]=new Font("SansSerif", 1, fs);
  BUKgarf.fonts[1]=new Font("SansSerif", 1, fs-3);
  BUKgarf.fonts[2]=new Font("SansSerif", 1, fs-3);

 g.setFont(new Font("Dialog",1,fs));
 FontMetrics F=g.getFontMetrics();

 BUKgarf.drawString(nach,xx,yy);
  ...
  if (strD[i].equals("@"))
    {
     if(i>0)
     {
      BUKgarf.drawString(strZ[i-1]+" ",size,yy);
      size=size+F.stringWidth(strZ[i-1]+" ");
     }
     BUKgarf.drawString(strU[i],size,yy);
    ...
    else
    {
     ...
     BUKgarf.drawString(strU[i],size+((dl-size1)/2)+1,yy-8);
     BUKgarf.drawString(strD[i],size+((dl-size2)/2)+1,yy+9);
