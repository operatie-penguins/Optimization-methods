...
String U[], String D[], String strZ[],int fs, int count, int xx, int yy)
{
 ...
 int size=0, size1=0, size2=0, dl;
  ...
  Stringer4.fonts[0]=new Font("SansSerif", 1, fs);
  Stringer4.fonts[1]=new Font("SansSerif", 1, fs-3);
  Stringer4.fonts[2]=new Font("SansSerif", 1, fs-3);

 g.setFont(new Font("Dialog",1,fs));
 FontMetrics F=g.getFontMetrics();

 Stringer4.drawString(nach,xx,yy);
  ...
  if (strD[i].equals("@"))
    {
     if(i>0)
     {
      Stringer4.drawString(strZ[i-1]+" ",size,yy);
      size=size+F.stringWidth(strZ[i-1]+" ");
     }
     Stringer4.drawString(strU[i],size,yy);
    ...
    else
    {
     ...
     Stringer4.drawString(strU[i],size+((dl-size1)/2)+1,yy-8);
     Stringer4.drawString(strD[i],size+((dl-size2)/2)+1,yy+9);
