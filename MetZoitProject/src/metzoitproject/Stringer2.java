...
public class Stringer2 {
        static Graphics2D g2d;
        static Font[] fonts = {
                new Font("SansSerif", 1, 12),	// основной текст
                new Font("SansSerif", 1, 10),   // глваный индекс
                new Font("SansSerif", 1, 10)     // мелкие индексы
        };
...
//------------------------------------------------------------------------------
public static void drawDrob(Graphics g, String nach, String N, String strU[], String strD[],
String U[], String D[], String strZ[],int fs, int count, int xx, int yy)
{
 Stringer2.setGraphics(g);
 int size=0, size1=0, size2=0, dl;


  Stringer2.fonts[0]=new Font("SansSerif", 1, fs);
  Stringer2.fonts[1]=new Font("SansSerif", 1, fs-3);
  Stringer2.fonts[2]=new Font("SansSerif", 1, fs-3);

 g.setFont(new Font("Dialog",1,fs));
 FontMetrics F=g.getFontMetrics();

 Stringer2.drawString(nach,xx,yy);
 size=xx+F.stringWidth(N+" ");
  ...
  if (strD[i].equals("@"))
    {
     if(i>0)
     {
      Stringer2.drawString(strZ[i-1]+" ",size,yy);
      size=size+F.stringWidth(strZ[i-1]+" ");
     }
     Stringer2.drawString(strU[i],size,yy);
     size=size+F.stringWidth(U[i]+" ");
    ...
    else
    {
     ...
     size1=F.stringWidth(U[i]);
     size2=F.stringWidth(D[i]);
     if (size1>=size2) dl=size1+8; else dl=size2+8;
     g.drawLine(size,yy-5,size+dl,yy-5);
     Stringer2.drawString(strU[i],size+((dl-size1)/2)+1,yy-8);
     Stringer2.drawString(strD[i],size+((dl-size2)/2)+1,yy+9);
     size=size+dl+F.stringWidth(" ");
